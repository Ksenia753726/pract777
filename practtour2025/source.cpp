...
void FillFileWithMark(std::string& filename, Student* students, int32_t size) {
	...
	for (size_t i = 0; i < size; ++i)
	{
		...
		fout_mark << "ÃÅÎ;" << students[i].mark_geo << ';';
		fout_mark << "ÏÐÎÃ;" << students[i].mark_prog << '\n';
	}
	fout_mark.close();
}
...
void MakeBinaryFileWithSurnAndMarks(Student* students, int32_t size) {
	std::ofstream fout_SMBIN("D:/прога/дз/practtour2025/SurnamesMarks.bin", std::ios::binary);
...
void MakeBinaryFileToTextFile(std::fstream& bin_file) {
	...
	while (!bin_file.eof()) {
		size_t length;
		bin_file.read((char*)&length, sizeof(length));
		if (bin_file.eof())
		{ 
			break;
		}
		std::string line;
		bin_file.read(&line[0], length);
